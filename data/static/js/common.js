...
{number=formBuy.elements['number'].value;}
quick=1;}
goods.quick=quick;goods.spec=spec_arr;goods.goods_id=goodsId;goods.number=number;goods.parent=(typeof(parentId)=="undefined")?0:parseInt(parentId);Ajax.call('flow.php?step=add_to_cart','goods='+goods.toJSONString(),addToCartResponse_quick,'POST','JSON');}
...
{if(result.error>0)
{if(result.error==2)
{swal({title:'Out of stock (Notify)',text:"Selected product attribute is out of stock would you like to add this product to your arrival notice so you can receive notification when this product is back in stock",showCancelButton:true,confirmButtonColor:'#ffffff',cancelButtonColor:'#ffffff',background:'rgba(0,0,0,.7)',cancelButtonText:'No',confirmButtonText:'Yes'}).then(function(){window.location.href='user.php?act=add_booking&id='+result.goods_id+'&spec='+result.product_spec;}).catch(swal.noop);}
else if(result.error==6)
{openSpeDiv(result.message,result.goods_id,result.parent);}
else
{alert(result.message);}}
else
{document.getElementById('carId').innerHTML=result.cart_number;document.getElementById('globalId').innerHTML=result.cart_number;showDiv();}}
...
package_info.number=number;Ajax.call('flow.php?step=add_package_to_cart','package_info='+package_info.toJSONString(),addPackageToCartResponse,'POST','JSON');}
...
goods.quick=quick;goods.spec=spec_arr;goods.goods_id=goods_id;goods.number=number;goods.parent=(typeof(parentId)=="undefined")?0:parseInt(parentId);Ajax.call('flow.php?step=add_to_cart','goods='+goods.toJSONString(),addToCartResponse,'POST','JSON');document.body.removeChild(docEle('speDiv'));document.body.removeChild(docEle('mask'));var i=0;var sel_obj=document.getElementsByTagName('select');while(sel_obj[i])
